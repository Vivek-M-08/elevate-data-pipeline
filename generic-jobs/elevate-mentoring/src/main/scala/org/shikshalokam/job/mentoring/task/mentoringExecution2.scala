...
object mentoringExecution2 {
  def main(args: Array[String]): Unit = {
    // Check if the correct number of arguments are provided
    if (args.length != 2) {
      println("Please provide the input parameters \n Usage: MySparkApp <table_name> <script_type>")
      System.exit(1)
    }
    ...
    val app = new mentoringExecution2(table_name, script_type)
    // Run the application
    app.process()
